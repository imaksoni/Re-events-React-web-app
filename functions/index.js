    ...
    try {
      ...
      batch.set(
          ...
          .doc(context.params.userUid),
        {
          ...
          photoURL: userDoc.data().photoURL || '/assets/user.png',
          uid: userDoc.id,
        }
      );
          ...
          let attendeeJoined = after.attendees.filter(item1 => !before.attendees.some(item2 =>
            ...
            try{
              ...
              followerDocs.forEach(doc => {
                admin.database().ref(`/posts/${doc.id}`).push(newPost(attendeeJoined,
                  'joined-event', context.params.eventId, before))
              })
            } catch(error){
              return console.log(error);
            }
        ...
        if(before.attendees.length > after.attendees.length) {
          let attendeeLeft = before.attendees.filter(item1 => !after.attendees.some(item2 =>
            ...
            try{
              ...
              .collection('userFollowers').get();
              followerDocs.forEach(doc => {
                admin.database().ref(`/posts/${doc.id}`).push(newPost(attendeeLeft,
                  'left-event', context.params.eventId, before))
              })
            } catch(error){
              return console.log(error);
            }
